...
public class PantallaAceptarCursos extends JFrame {
	...
	public PantallaAceptarCursos() throws Exception {
		...
		Vector<Object> a = cDAO.seleccionarCursos("SELECT id,nombre,ECTS,centro,director,tipo FROM CursoPropio WHERE `estado`= 'PROPUESTO'");
                	...
                	btnAceptar.addActionListener((ActionListener) new ActionListener() {
            			public void actionPerformed(ActionEvent e) {
            				try {
            					...
            					String b=a.get(list.getSelectedIndex()).toString().replace("[", "").split(",")[0];
            					cDAO.editarCurso("UPDATE CursoPropio SET `estado`= 'VALIDADO' WHERE `id` = '"+b+"'");
            					listModel.remove(list.getSelectedIndex());
            				} catch (Exception e1) {
            					e1.printStackTrace();
            				}
